...
<h2>This will only hide the plugin from the plugins page, it will not remove menu items that are associated with the plugin</h2>
<form name="plugin_hiding_form" method="post" action="/wp-admin/admin-post.php">
	<style>
	...
	$active_plugins = get_option($plugin_settings['options_prefix']."hidden_plugins");
	...
	if ( isset( $notoptions[$plugin_settings['options_prefix'].'hidden_plugins'] ) ) {
	...
	<?php foreach($all_plugins as $key=>$plugin): ?>
        ...
        <tr valign="top">
	        ...
	        <td>
			<input type="checkbox" class="onoff plugin" name='<?php echo $plugin_settings['options_prefix']; ?>hidden_plugins[<?php echo $key; ?>]' <?php echo $checked ? "checked='checked'" : '' ?> />
	       	 <em>Visibility for <?php echo $plugin["Name"]; ?></em>
	        </td>
        </tr>
        <?php $pluginCounter++; ?>
	<?php endforeach; ?>
    ...
    <input type="hidden" name="command" value="update_plugin_hiding">
